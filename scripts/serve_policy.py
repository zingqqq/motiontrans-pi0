...
def create_eval_default_policy(env: EnvMode, *, default_prompt: str | None = None) -> _policy.Policy:
    """Create a default policy for the given environment."""
    if checkpoint := DEFAULT_CHECKPOINT.get(env):
        return _policy_config.create_eval_policy(
            _config.get_config(checkpoint.config), checkpoint.dir, default_prompt=default_prompt
        )
    raise ValueError(f"Unsupported environment mode: {env}")


def create_eval_policy(args: Args, config: _config.TrainConfig) -> _policy.Policy:
    match args.policy:
        case Checkpoint():
            print(f"Use ckpt: {args.policy.dir}")
            return _policy_config.create_eval_policy(config, args.policy.dir, default_prompt=args.default_prompt)
        case Default():
            return create_eval_default_policy(args.env, default_prompt=args.default_prompt)
